...
find_skill_files() {
  ...
  # -L follows symlinks (user skills are often symlinked)
  find -L "$SEARCH_ROOT" -type f -path "*/skills/*/SKILL.md" 2>/dev/null || true
}
...
# Extract explicitly requested skills via +skill:name syntax
...
# Main execution
EXPLICIT_SKILLS=$(extract_explicit_skills "$TASK_DESC")
...
# Merge explicit skills (they always come first with score 999)
if [[ -n "$EXPLICIT_SKILLS" ]]; then
  SCORED="$EXPLICIT_SKILLS
$SCORED"
fi
...
# Sort by score (descending), deduplicate by name (keep highest score), take top N, format as JSON
# FIX (Skeptic Mirage 1): Filter out empty lines before JSON generation
echo "$SCORED" | grep -v '^$' | sort -t$'	' -k1 -nr | awk -F'	' '!seen[$2]++ {print}' | head -n "$MAX_RESULTS" | awk -F'	' '
BEGIN {
  printf "["
  first = 1
}
